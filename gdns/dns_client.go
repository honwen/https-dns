...
func ParseEndpoint(endpoint string, defaultPort int) (ep Endpoint, err error) {
	...
	if len(e) > 1 {
		i, err := strconv.ParseInt(e[1], 10, 16)
		if err != nil {
...
func (c *SimpleDNSClient) LookupIP(host string) ([]net.IP, error) {
	...
	if ok && entry.expires.After(time.Now()) {
		// glog.V(LDEBUG).Infof("simple dns cache hit for %v", host)
	...
	glog.V(LINFO).Infof("simple dns lookup %v", host)

	r, _, err := exchange(&msg, server.String())
	if err != nil {
		return []net.IP{}, err
	}
